...
@Component({
  selector: 'app-custom-role-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, PoModalModule, PoFieldModule],
  templateUrl: './custom-role-modal.component.html',
})
export class CustomRoleModalComponent
  extends BaseModalComponent<{ item?: CustomRoleResponse }, { confirmed: boolean }>
  implements OnInit
{
  ...
  save(): void {
    ...
    const operation: Observable<string | void> = this.data?.item
      ? this.service.updateCustomRole(this.data.item.id, request)
      : this.service.createCustomRole(request);
